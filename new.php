...
<?php
  ...
  window.onclick = function (event) {
    ...
    <div class="category-tabs">
      <a href="#best-sellers" class="active">베스트 셀러</a>
    ...
    if($result && $result->num_rows>0){
      for($i=0;$i<4;$i++){
        ...
        <a href="best/b<?=$row['id']?>.php?isLogged=<?=$isLogged?>&name=<?=$row['name']?>&price=<?=$row['price']?>&photo=<?=$row['photo']?>">
        <div class="product-item">
          <img src="best/b/<?=$row['photo']?>" alt="상품 이미지 1" />
          <p><?=$row['name']?> </p>
