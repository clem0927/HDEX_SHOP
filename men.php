...
<?php
  ...
  window.onclick = function (event) {
    ...
    <div class="category-tabs">
      <a href="#shirts" class="active">셔츠</a>
      <a href="#pants">바지</a>
    ...
    if($result && $result->num_rows>0){
      for($i=0;$i<4;$i++){
        ...
        <a href="men/m<?=$row['id']?>.php?isLogged=<?=$isLogged?>&name=<?=$row['name']?>&price=<?=$row['price']?>&photo=<?=$row['photo']?>">
        <div class="product-item">
          <img src="men/m/<?=$row['photo']?>" alt="상품 이미지 1" />
          <p><?=$row['name']?> </p>
